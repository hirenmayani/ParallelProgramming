...
struct task_queue {
	...
	void push_back(mmArgs2 mma) {
		l.lock();
		pending.push_back(mma);
		l.unlock();
	}

	void push_front(mmArgs2 mma) {
		l.lock();
...
void parRecMM_SC1(mmArgs2 mmaWrapper, int stype, int myid) {
	mmArgs mma= mmaWrapper.curr;
	...
	if (mma.currSize == mma.minSize) {
		...
		mmArgs2 mma1W,mma2W,mma3W,mma4W;
		mma1W.prev = mma;
		mma1W.curr = mma1;
		mma1W.syncC = &syncC;
		...
		if (stype == 3) {
			ddq.push_front(mma1W);
			ddq.push_front(mma2W);
			ddq.push_front(mma3W);
		} else if(stype == 2) {
			tqs[rand() % nthreads].push_front(mma1W);
			tqs[rand() % nthreads].push_front(mma2W);
			tqs[rand() % nthreads].push_front(mma3W);
		} else if(stype == 1) {
			tqs[myid].push_front(mma1W);
			tqs[myid].push_front(mma2W);
			tqs[myid].push_front(mma3W);
		}
...
void scheduler(int r, int** Z, int** X, int** Y, int minSize, int sType) {
	...
	ddq.push_back(aw);
	tqs[0].push_back(aw);
	work[0]+= int n = pow(2, mma.currSize);
    for (int i = 0; i < nthreads; i++) {
            if (sType == 1)
