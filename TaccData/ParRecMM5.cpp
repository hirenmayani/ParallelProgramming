...
void parRecMM_SC(mmArgs a, bool cshare);
int emptyQs = 0;
...
void CShare(){
		unsigned int atmps =0;
		while (true){
			mmArgs mma = ddq.cpop_back();
                        if (mma.currSize >  0){
                                atmps = 0;
                                parRecMM_SC(mma,true);
                        }
                        else{
				atmps += 1;
				if(atmps == 99999){
...
void parRecMM_SC(mmArgs mma, bool cshare)
{
