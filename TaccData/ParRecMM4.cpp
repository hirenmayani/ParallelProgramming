...
void indexMM(int zi, int zj, int xi, int xj, int yi, int yj, int currSize, int** Z, int** X, int** Y)
{
	...
	for(i=0;i<n;i++)
				for(j=0;j<n;j++)
						for(k=0;k<n;k++)
			{
                              //cout <<"Values" <<zi+i<<"\t"<<zj+j<<"\t"<<xi+i<<"\t"<<xj+k<<"\t"<<yi+k<<"\t"<<yj+j<<"\n";
                                                    //      cout<<"Base Address \t"<<Z<<"\t"<<X<<"\t"<<Y<<"\n";
                                                                              //      cout<<"Current Address \t"<<((Z+(zi+i))+(zj+j))<<"\t"<<((X+(xi+i))+(xj+k))<<"\t"<<(Y+(yi+k)+(yj+j))<<"\n";
...
void parRecMM_SC(mmArgs mma)
{
		...
		if(mma.currSize == mma.minSize){
			...
			mmArgs mma1 = mma;
			tqs[rand()%nthreads].pending.push_front(mma1);

			mmArgs mma2 = mma;
			mma2.zj+=n;
			mma2.yj+=n;
			tqs[rand()%nthreads].pending.push_front(mma2);

			mmArgs mma3 = mma;
			mma3.zi+=n;
			mma3.xi+=n;
			tqs[rand()%nthreads].pending.push_front(mma3);
			...
			mma4.yj+=n;
				...
				mma5.xj+=n;
				mma5.yi+=n;
					tqs[rand()%nthreads].pending.push_front(mma5);
				...
				mma6.yj+=n;
				mma6.zj+=n;
							tqs[rand()%nthreads].pending.push_front(mma6);
				...
				mma7.yi+=n;
				mma7.zi+=n;
							tqs[rand()%nthreads].pending.push_front(mma7);
 ...
 * 				/cilk_spawn DRSteal(tqs, i) ;*/
	...
	cilk_spawn DRSteal(tqs,nthreads-1);
